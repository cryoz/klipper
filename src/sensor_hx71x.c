           ...
           uint8_t force_flush)
{
    ...
    if (hx71x->sb.data_count + BYTES_PER_SAMPLE > ARRAY_SIZE(hx71x->sb.data) || force_flush)
...
{
    struct hx71x_adc *hx71x = oid_alloc(args[0], command_config_hx71x, sizeof(*hx71x));
...
void hx71x_attach_load_cell_probe(uint32_t *args)
{
    uint8_t oid = args[0];
...
void command_query_hx71x_status(const uint32_t *args)
{
    uint8_t oid = args[0];
    struct hx71x_adc *hx71x = oid_lookup(oid, command_config_hx71x);
    irq_disable();
