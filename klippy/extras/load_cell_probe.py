...
class ForceLine(object):
    ...
    def angle(self, line, time_scale=0.001):
        radians = (math.atan2(self.slope * time_scale, 1) - math.atan2(
            line.slope * time_scale, 1))
...
class ForceGraph:
    ...
    def tap_decompose(self, homing_end_time, pullback_start_time,
            pullback_cruise_time, pullback_cruise_duration, discard=0):
        ...
        if use_curve_optimization:
            ...
            l5 = self.line(break_contact_idx, pullback_end_idx, discard,
                discard)
            ...
            l4 = self.line(pullback_cruise_idx,
                break_contact_idx)  # TODO: surface this to users in the tap
            # results
...
TOO_FEW_PROBING_MOVES = (
    'TOO_FEW_PROBING_MOVES', '5 Probing moves expected but there were fewer')
TOO_MANY_PROBING_MOVES = (
    'TOO_MANY_PROBING_MOVES', '5 Probing moves expected but there were more')
...
TAP_BREAK_CONTACT_TOO_EARLY = (
    'TAP_BREAK_CONTACT_TOO_EARLY', 'Tap break-contact time too early, invalid')
TAP_BREAK_CONTACT_TOO_LATE = (
    'TAP_BREAK_CONTACT_TOO_LATE', 'Tap break-contact time too late, invalid')
...
class TapAnalysis(object):
    ...
    def get_toolhead_position(self, print_time):
        for i, move in enumerate(self.moves):
            ...
            if start_time <= print_time < end_time:
                ...
                dist = ((
                                move.start_v + .5 * move.accel * move_time) *
                        move_time)
                pos = ((move.start_x + move.x_r * dist,
                move.start_y + move.y_r * dist, move.start_z + move.z_r * dist))
                return pos
            else:
                continue
    ...
    def _try_analysis(self):
        ...
        self.pullback_end_time = (
                self.moves[PULLBACK_END].print_time + self.moves[
            PULLBACK_END].move_t)
        self.pullback_cruise_time = self.moves[PULLBACK_CRUISE].print_time
        self.pullback_duration = (
                self.pullback_end_time - self.pullback_start_time)
        ...
        self.r_squared_widths = [int((n * 0.01) // self.sample_time) for n in
            range(2, 7)]
    ...
    def validate_order(self):
        p = self.tap_points
        if not (p[0].time < p[1].time < p[2].time < p[3].time < p[4].time < p[
            5].time):
    ...
    def to_dict(self):
        return {
            ...
            'tap_angles': self.tap_angles, 'elapsed': self.elapsed,
...
class TappingMove:
    def __init__(self, config, load_cell_probing_move, tap_classifier_module,
            config_helper):
        self._printer = config.get_printer()
