...
class ForceGraph:
    ...
    def tap_decompose(self,
        ...
        l5 = self.line(break_contact_idx, pullback_end_idx, discard, discard)
        ...
        if use_curve_optimization:
            ...
            l4 = self.line(pullback_cruise_idx, break_contact_idx)  # TODO: surface this to users in the tap
            # results

        # Line intersections:
        p0 = ForcePoint(self.time[homing_start_idx], l1.find_force(self.time[homing_start_idx]))
        p1 = l1.intersection(l2)
        p2 = l2.intersection(l3)
        p3 = l3.intersection(l4)
        p4 = l4.intersection(l5)
        p5 = ForcePoint(self.time[pullback_end_idx], l5.find_force(self.time[pullback_end_idx]))
...
TOO_FEW_PROBING_MOVES = ('TOO_FEW_PROBING_MOVES', '5 Probing moves expected but there were fewer')
TOO_MANY_PROBING_MOVES = ('TOO_MANY_PROBING_MOVES', '5 Probing moves expected but there were more')
...
TAP_BREAK_CONTACT_TOO_EARLY = ('TAP_BREAK_CONTACT_TOO_EARLY', 'Tap break-contact time too early, invalid')
...
class TapAnalysis(object):
    ...
    def analyze(self, report_errors=True):
        t_start = time.time()
        try:
            self._try_analysis()
    ...
    def _try_analysis(self):
        ...
        self.pullback_end_time = (self.moves[PULLBACK_END].print_time + self.moves[PULLBACK_END].move_t)
        self.pullback_cruise_time = self.moves[PULLBACK_CRUISE].print_time
        self.pullback_duration = (self.pullback_end_time - self.pullback_start_time)
        self.position = self._extract_pos_history()

        points, lines = self.force_graph.tap_decompose(self.home_end_time, self.pullback_start_time,
                                                       self.pullback_cruise_time, self.pullback_duration)
...
class ParamHelper:
    ...
    def _get_choice(self, config, gcmd):
        ...
        if gcmd:
            ...
            return config.getchoice(name, self.choices, default=self.default_choice)
...
class LoadCellProbeConfigHelper:
    ...
    def __init__(self, config, load_cell_inst):
        ...
        self._horizontal_speed = floatParamHelper(config, 'horizontal_speed', above=0.1, default=50.)
...
class TappingMove:
    ...
    def __init__(self, config, load_cell_probing_move, tap_classifier_module, config_helper):
        self._printer = config.get_printer()
        self._load_cell_probing_move = load_cell_probing_move
        self._tap_classifier_module = tap_classifier_module
        self._config_helper = config_helper
    ...
    def run_tap(self, gcmd, report_errors=True):
        ...
        ppa = TapAnalysis(self._printer, samples)
        ppa.analyze(report_errors)
        # allow the TapClassifier to override TapAnalysis result
        self._tap_classifier_module.classify(ppa)
        # broadcast tap event data:
        self._clients.send({'tap': ppa.to_dict()})
        self._is_last_result_valid = ppa.is_valid
        if ppa.is_valid:
...
class TapSession:
    ...
    def _horizontal_move(self, location, gcmd, toolhead):
        ...
        toolhead.manual_move([x, y, None], self._config_helper.get_horizontal_speed(gcmd))

    def _move_right(self, gcmd, toolhead):
        pos = toolhead.get_position()
        toolhead.manual_move([pos[0] + 2.0, pos[1], None], self._config_helper.get_horizontal_speed(gcmd))
...
class LoadCellPrinterProbe:
    ...
    def __init__(self, config):
        ...
        # Read all user configuration and resolve plugin modules
        ...
        self._tapping_move = TappingMove(config, load_cell_probing_move, tap_classifier, config_helper)
