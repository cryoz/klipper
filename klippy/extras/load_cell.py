...
class LoadCellGuidedCalibrationHelper:
    ...
    def __init__(self, printer, load_cell):
        ...
        self.gcode.respond_info("Starting load cell calibration. \n"
    ...
    def finalize(self, save_results=False):
        ...
        self.gcode.respond_info("Load cell calibration settings:\n\n"
                                ...
                                " with the above and restart the printer." % (self._counts_per_gram, self._tare_counts))
    ...
    def cmd_CALIBRATE(self, gcmd):
        ...
        gcmd.respond_info("Calibration value: %.2f%% (%i), Counts/gram: %.5f, \
            Total capacity: +/- %0.2fKg" % (cal_percent, cal_counts, c_per_g, cap_kg))
        range_min, range_max = self.load_cell.saturation_range()
        ...
        if (abs(cal_percent - self.tare_percent)) < 1.:
            raise self.printer.command_error("ERROR: Tare and Calibration readings are less than 1% "
                                             "different!\n"
    ...
    def _collect_until(self, timeout):
        ...
        while self.is_started:
            ...
            if self._mcu.estimated_print_time(now) > timeout:
                ...
                raise self._printer.command_error("LoadCellSampleCollector timed out! Errors: %i,"
...
class LoadCell:
    ...
    def _sensor_data_event(self, msg):
        ...
        for row in data:
            ...
            samples.append([row[0], self.counts_to_grams(row[1]), row[1], self.tare_counts])
    ...
    def avg_counts(self, num_samples=None):
        ...
        if errors:
            raise self.printer.command_error("Sensor reported %i errors while sampling" % (errors[0] + errors[1]))
